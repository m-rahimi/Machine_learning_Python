...
def load_data_sparse(data_path=""):
    ...
    train_file = data_path + "../data/train.json"
    test_file = data_path + "../data/test.json"
    ...
    train_df['sum4'] = dis[['dis0', 'dis1', 'dis2', 'dis3', 'dis4']].sum(axis=1)
    train_df['sum3'] = dis[['dis0', 'dis1', 'dis2', 'dis3']].sum(axis=1)
    train_df['sum2'] = dis[['dis0', 'dis1', 'dis2']].sum(axis=1)
    train_df['sum1'] = dis[['dis0', 'dis1']].sum(axis=1)
    
    test_df['sum4'] = dis[['dis0', 'dis1', 'dis2', 'dis3', 'dis4']].sum(axis=1)
    test_df['sum3'] = dis[['dis0', 'dis1', 'dis2', 'dis3']].sum(axis=1)
    test_df['sum2'] = dis[['dis0', 'dis1', 'dis2']].sum(axis=1)
    test_df['sum1'] = dis[['dis0', 'dis1']].sum(axis=1)
    ...
    features_to_use.extend(["bathrooms", "bedrooms", "latitude", "longitude", "price"])
    features_to_use.extend(['sum1', 'sum2', 'sum3', 'sum4'])
    features_to_use.extend(image_date.columns)
...
def main():
        ...
        X,X_test,y,ids=load_data_sparse (data_path="")# you might need to change that to whatever folder the json files are in
...
#        CO=[0,14,21] # columns to create averages on
        CO=[3,17,24] # columns to create averages on
        ...
        param['silent'] =  1
        param['nthread'] = 16 # put more if you have
